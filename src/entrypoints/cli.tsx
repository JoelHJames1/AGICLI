...
async function main(): Promise<void> {
  ...
  // Fast-path for `--whatsapp`: start the WhatsApp bridge.
  // Connects Claude2 to WhatsApp via QR code authentication.
  if (args[0] === '--whatsapp' || args[0] === 'whatsapp') {
    profileCheckpoint('cli_whatsapp_path');
    const { startWhatsAppCLI } = await import('../channels/whatsapp/cli.js');
    await startWhatsAppCLI({
      qrOnly: args.includes('--qr-only'),
      model: (() => {
      ...
      allowedSenders: (() => {
        ...
        return idx !== -1 && args[idx + 1] ? args[idx + 1]!.split(',') : undefined;
      })(),
    });
    return;
  }
